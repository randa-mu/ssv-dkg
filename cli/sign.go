...
func Sign(config api.SignatureConfig, log shared.QuietLogger) (api.SigningOutput, error) {
	...
	depositDataMessage, err := crypto.DepositMessageRoot(config.DepositData.IntoMessage(shared.Clone(groupPublicKey)), config.DepositData.ForkVersion)
	if err != nil {
...
func signatureResponseVerifies(suite crypto.ThresholdScheme, identity crypto.Identity, depositData api.UnsignedDepositData, owner api.OwnerConfig, response api.SignResponse) error {
	...
	message, err := crypto.DepositMessageRoot(depositData.IntoMessage(shared.Clone(groupPublicKey)), depositData.ForkVersion)
	if err != nil {
