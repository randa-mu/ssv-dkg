...
func Sign(config api.SignatureConfig, log shared.QuietLogger) (api.SigningOutput, error) {
	...
	depositDataMessage, err := crypto.DepositDataMessage(config.DepositData.ExtractRequired(), config.DepositData.ForkVersion, shared.Clone(groupPublicKey))
	if err != nil {
...
func signatureResponseVerifies(suite crypto.ThresholdScheme, identity crypto.Identity, depositData api.UnsignedDepositData, owner api.OwnerConfig, response api.SignResponse) error {
	...
	message, err := crypto.DepositDataMessage(depositData.ExtractRequired(), depositData.ForkVersion, shared.Clone(groupPublicKey))
	if err != nil {
