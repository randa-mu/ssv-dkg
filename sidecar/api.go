...
func (d Daemon) Sign(request api.SignRequest) (api.SignResponse, error) {
	...
	depositDataMessage, err := crypto.DepositMessageSignatureMessage(request.DepositData.IntoMessage(shared.Clone(groupPublicKey)), request.DepositData.ForkVersion)
	if err != nil {
		return api.SignResponse{}, err
	}
